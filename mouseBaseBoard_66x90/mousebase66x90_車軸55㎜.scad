...
module mousemount(){
  ...
  difference(){
    hull(){
      ...
      translate([x/2, -y/2, 0]) cube([-endr+x/2,y,h]);
      translate([90-(endr/2), -(y/2)+(endr/2), 0]) cylinder(h=1.6, r=(endr/2));
      translate([90-(endr/2), (y/2)-(endr/2), 0]) cylinder(h=1.6, r=(endr/2));
    }
  ...
  /*
  //ここから検証用
  translate([-10, 0, 5]) rotate([0,0,90])text("Axis55mm");
  //寸法補助バー
  //color("red") translate([0, -y/2, -5]) cube([90,y,h]);
  color("pink") translate([0, -10-y/2, 2]) cube([shax, 2, 2]);
  //color("blue") translate([0, -y/2+0.5, 1.6]) cube([45,1.5,1+(17/2)]);
  //color("green") translate([0, -y/2+0.5, 1.6+(19/2)]) cube([45,1.5,1+(17/2)]);

  translate([0,-1,15])cube([shax,2,2]);

  color("pink"){
    translate([mmx, -mmy1+0.1, 0.5]) 1717mount_mount();
    rotate([0,0,180]) mirror([1,0,0])translate([mmx, -mmy1+0.1, 0.5]) 1717mount_mount();

    translate([mmx-3,(y/2)-20,1.6+4]) rotate([0,0,90])connector1717();
    rotate([0,0,-180])mirror([1,0,0]) translate([mmx-3,(y/2)-20,1.6+4]) rotate([0,0,90])connector1717();
    translate([mmx-13,-15/2,0])rotate([0,0,0])motordriver_DRV8835();

//CPU
    translate([shax-5, -(21/2), 4+8+1.6])cube([35,21,1.6]);

    translate([shax,40,15])rotate([90,0,0])cylinder(h=80, r=1.5);
    translate([shax,-11,0])rotate([90,0,0])battery();
    translate([90-8,-23,0])rotate([0,0,0])ph2housing();
    translate([90-8,20,0])rotate([0,0,0])ph2housing();
    translate([89,-8.9/2,1.6])rotate([0,0,90])sw1();

  }
    translate([9,2,16.7+1.6])rotate([-90,0,45]) sensor_case(8,10, 16.8, 4, 3.6, 4, 11);
    translate([15,-8,16.7+1.6])rotate([-90,0,-45-180]) sensor_case(8,10, 16.8, 4, 3.6, 4, 11);
    translate([mmx+4,-(y/2)+2,16.7+1.6])rotate([-90,0,90]) sensor_case(8,10, 16.8, 4, 3.6, 4, 11);
    translate([mmx+4,(y/2)-2-8,16.7+1.6])rotate([-90,0,-270]) sensor_case(8,10, 16.8, 4, 3.6, 4, 11);
    */
}